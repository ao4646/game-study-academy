...
async function getCategoryArticles(): Promise<ArticleWithRelations[]> {
  try {
    const { data: articleCategoryRelations, error: articlesError } = await supabase
      ...
      `)
      .eq('category_id', 21) // スーパープレイ(RTAなど)カテゴリ
...
export function generateMetadata(): Metadata {
  return {
    title: 'エルデンリング スーパープレイ(RTAなど) 記事一覧 - Game Study Academy',
    description: 'エルデンリング（Elden Ring）のスーパープレイ記事一覧。RTA（リアルタイムアタック）、速攻、縛りプレイ、世界記録、テクニカルプレイなどの超人技を網羅。',
    keywords: [
      'エルデンリング',
      'Elden Ring',
      'スーパープレイ',
    ...
    openGraph: {
      type: 'website',
      locale: 'ja_JP',
      url: 'https://game-study-academy.com/games/elden-ring/category21',
      ...
      images: [
        {
          ...
          width: 1200,
          height: 630,
          alt: 'Game Study Academy - エルデンリング スーパープレイ',
        },
      ],
    },
    twitter: {
    ...
    alternates: {
      canonical: 'https://game-study-academy.com/games/elden-ring/category21',
    },
  }
}
...
function ArticleCard({ data }: { data: ArticleWithRelations }) {
  ...
  const categoryName = categories.length > 0 ? categories[0].name : 'スーパープレイ'
...
// メインコンポーネント
export default async function SuperPlayPage() {
  ...
  const breadcrumbItems = [
    ...
    { name: 'スーパープレイ', url: 'https://game-study-academy.com/games/elden-ring/category21' }
  ]
  ...
  return (
    <>
      ...
      <div className="min-h-screen bg-gray-50">
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
            <div className="text-center">
              ...
              <p className="text-xl md:text-2xl mb-8 max-w-3xl mx-auto leading-relaxed">
                エルデンリング（Elden Ring）のスーパープレイ記事一覧。<br />
                RTA（リアルタイムアタック）、速攻、縛りプレイなど、超人技の数々を紹介しています。
              </p>
              <div className="flex flex-col sm:flex-row gap-4 justify-center">
                <Link
                  href="/games/elden-ring"
                  className="bg-white text-violet-600 px-8 py-3 rounded-full font-semibold text-lg transition-all duration-300 hover:bg-gray-100 hover:scale-105"
                >
                  🔰 エルデンリング記事一覧
                </Link>
                <Link
                  href="/articles"
                  className="border-2 border-white text-white px-8 py-3 rounded-full font-semibold text-lg transition-all duration-300 hover:bg-white hover:text-violet-600"
                >
                  📚 全記事一覧
                </Link>
              </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          <nav className="flex" aria-label="Breadcrumb">
            <ol className="flex items-center space-x-4">
              ...
              <li>
                <span className="text-gray-900 font-medium">スーパープレイ</span>
              </li>
            </ol>
          </nav>
        </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          {articles.length > 0 ? (
            ...
            <section className="text-center py-16">
              ...
              <h2 className="text-2xl font-bold text-gray-900 mb-4">スーパープレイ記事準備中</h2>
              <p className="text-gray-600 mb-8">
                スーパープレイ記事は現在準備中です。<br />
                順次追加予定ですので、お楽しみに！
              </p>
              <Link
                href="/games/elden-ring"
        ...
        {/* Call to Action */}
        <section className="bg-gradient-to-r from-violet-600 to-violet-700 text-white mt-16">
          <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12 text-center">
            <h2 className="text-3xl font-bold mb-4">
              🏆 超人技を目撃しよう！
            </h2>
            <p className="text-xl mb-8 leading-relaxed">
              スーパープレイ記事で驚異的なテクニックを学びましょう。<br />
