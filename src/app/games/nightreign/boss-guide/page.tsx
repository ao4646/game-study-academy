...
export function generateMetadata(): Metadata {
  return {
    title: 'ナイトレイン 夜の王攻略 - Game Study Academy',
    description: 'エルデンリング：ナイトレインの夜の王攻略記事一覧。YouTube動画から学ぶボス戦の必勝法とテクニックを分かりやすく解説します。',
    keywords: [
      'ナイトレイン',
      'Nightreign',
      '夜の王',
    ...
    openGraph: {
      type: 'website',
      locale: 'ja_JP',
      url: 'https://game-study-academy.com/games/nightreign/boss-guide',
      ...
      images: [
        {
          ...
          width: 1200,
          height: 630,
          alt: 'Game Study Academy - ナイトレイン夜の王攻略',
        },
      ],
    },
    twitter: {
    ...
    alternates: {
      canonical: 'https://game-study-academy.com/games/nightreign/boss-guide',
    },
  }
}
    ...
    <Link href={`/articles/${article.id}`} className="block group">
      <article className="bg-white rounded-lg shadow-md overflow-hidden transition-all duration-300 hover:shadow-xl hover:-translate-y-1">
        <div className="relative aspect-video overflow-hidden">
          ...
          {boss && boss.weekpoints && boss.weekpoints.length > 0 && (
            <div className="absolute top-3 right-3">
              <span className="bg-black bg-opacity-70 text-white px-2 py-1 rounded text-xs">
                弱点: {boss.weekpoints.length}件
              </span>
            </div>
          )}
...
function BossCard({ boss, articleCount }: { boss: Boss, articleCount: number }) {
  return (
    <Link href={`/bosses/${boss.slug}`} className="block">
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4 transition-all duration-300 hover:shadow-md hover:-translate-y-1 cursor-pointer">
        <div className="flex items-center space-x-3">
          ...
          <div className="flex-1 min-w-0">
            <h4 className="text-lg font-bold text-gray-900 truncate hover:text-red-600 transition-colors">{boss.name}</h4>
...
export default async function BossGuidePage() {
  ...
  const breadcrumbItems = [
    ...
    { name: '夜の王攻略', url: 'https://game-study-academy.com/games/nightreign/boss-guide' }
  ]
